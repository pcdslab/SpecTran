...
from torch.nn.modules import Sequential, ModuleList, Linear
from torch.nn.modules.module import Module
from torch.nn.functional import log_softmax

__all__ = ['AdaptiveLogSoftmaxWithLoss']

_ASMoutput = namedtuple('_ASMoutput', ['output', 'raw_loss', 'mean_loss'])


# Adapted from the pytorch implementation (https://pytorch.org/docs/stable/_modules/torch/nn/modules/adaptive.html#AdaptiveLogSoftmaxWithLoss)
# updated to include ignore_index
class AdaptiveLogSoftmaxWithLoss(Module):
    ...
    * :attr:`head_bias` if set to True, adds a bias term to the 'head' of the
    ...
    def forward(self, input_: Tensor, target_: Tensor, ignore_index=None) -> _ASMoutput:
        ...
        output[ignore_mask] = 0

        relevant_output_for_loss_indices = output.nonzero()  
        relevant_output_for_loss = output[relevant_output_for_loss_indices]  
    
        # only consider mean of non-masked tokens since the rest are always 0
        mean_loss = (-relevant_output_for_loss).mean()
        ...
        return _ASMoutput(output, raw_loss, mean_loss)
